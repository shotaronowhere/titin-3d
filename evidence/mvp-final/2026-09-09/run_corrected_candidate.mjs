...
const checks = [
  ...['chromium','firefox'].map(engine=>[`${engine}-polish`, 'npx', ['playwright','test','test/browser/final-polish.spec.js',`--project=${engine}`]]),
  ['verify', 'npm', ['run', 'verify']],
  ['identity', 'npm', ['run', 'verify:identity']],
  ['chromium-routes', 'npx', ['playwright','test', ...['mvp-preview','evidence','workbench'].map(n=>`test/browser/${n}.spec.js`), '--project=chromium']],
  ['camera', 'npx', ['playwright','test','test/browser/ux-overhaul.spec.js','--project=chromium','--grep','semantic cameras clear chrome']],
  ...
  ['firefox-routes', 'npx', ['playwright','test', ...['mvp-preview','evidence'].map(n=>`test/browser/${n}.spec.js`), '--project=firefox']],
];
const results = [];
