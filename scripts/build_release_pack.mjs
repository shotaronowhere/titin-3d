...
function scientificAuthorityDoc(pack) {
  ...
  const lines = [
    ...
    '',
    `**Decision-ledger status:** ${authority.status}`,
    '',
    `**SC-21 mechanics status:** ${authority.mechanics_sprint_status}`,
    '',
    ...
    '| Field | Value |',
    '|---|---|',
    `| Parameter set | \`${authority.mechanics.parameter_set_id}\` |`,
    `| Model fingerprint | \`${authority.mechanics.model_fingerprint}\` |`,
    `| Decision | ${authority.mechanics.decision_id} ${authority.mechanics.decision_status} |`,
    `| Evaluation | ${authority.mechanics.evaluation_status}; force_pN = ${authority.mechanics.force_pN} |`,
    `| Approved supported range | ${authority.mechanics.approved_supported_range_nm ?? 'none'} |`,
    `| Sensitivity | ${authority.mechanics.sensitivity_status} |`,
    '',
    authority.mechanics.caveat,
    '',
    '| Decision | Status |',
    '|---|---|',
