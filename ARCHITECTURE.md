...
src/                       spec-consuming model, API, geometry, and renderer
  index.template.html      editable page source for the generated root index.html
...
npm run build                          # regenerate the self-contained index.html
npm run check:build                    # reject a stale committed index.html
npm run check:structures               # verify optional raw-source cache
npm run serve                          # then open http://localhost:8000/
```

The viewer is the generated `index.html` at the package root. It embeds Three.js,
the application module graph, and the scientific JSON records into one file. The
same artifact works from `file://`, any static server, and GitHub Pages with no
runtime dependencies. `src/index.template.html` is the editable page source;
`scripts/build_standalone.mjs` produces the root file, and the release gate compares
the generated bytes with the committed artifact so source/data changes cannot leave
a stale deployment behind.
