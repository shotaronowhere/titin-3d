...
Source candidate: `dcdb403a759bd3df1f7d7bc821f3a73aeeb10caa`. [Verified package and coverage record](/Users/shotaro/Downloads/artifacts/evidence/mvp-final/2026-09-08/PACKAGE.md). The instructions below remain as the execution record.

Both scientific presentation corrections, stable Stretch framing, the two desktop span labels, final evidence action, source placement, force ordering, native keyboard handling, return focus/scroll restoration, and the six SVG fallback repairs are complete. Additional opening/scaffold labels were cut as planned. The scientific model and all 24 protected data/dependency files are unchanged.

Verification passed 618 Node tests, 87 Chromium checks, and 34 Firefox checks. WebKit remains unverified after timeout stalls; the section 9.C two-engine fallback applies. A clean checkout reproduced 23 files exactly. The ZIP's 26 files and manifest entries verify, and all 20 extracted exports pass. The complete 22-frame native-click visual review identifies the exact delivered standalone/manifest bytes. Ten additional extracted desktop core-route captures were completed; later capture stalls prevented a complete fresh extracted phone/Architecture image set. These boundaries and the retained failed attempts are explicit in the package record. Independent scientific/human review and formal release readiness remain pending; `release_ready` stays false.
