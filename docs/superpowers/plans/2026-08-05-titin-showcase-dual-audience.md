...
6. **Size ceiling: 2,599,633 bytes for `index.html`** (baseline 2,166,361 × 1.20), enforced by `test/showcase_phase8.test.js`. The ceiling is derived, not written down: the test reads both factors from `data/release_gates.json` → `performance.baseline`. SC-17 Task 17.2 Step 1 moved it, rebaselining from the SC-7 figure (1,988,206, ceiling 2,385,847) to the SC-17 release point. The artifact **is** the baseline as of that step — 2,166,361 bytes — so the whole 20% tolerance, ≈ 423 KB, is headroom again. Check with `ls -l index.html` after each sprint.
